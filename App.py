...
class RejectedReservation(db.Model):
    id = db.Column(db.Integer, primary_key=True)
    name = db.Column(db.String(200), nullable=False)
    email = db.Column(db.String(200), nullable=False)
    phone = db.Column(db.String(20), nullable=False)
    ...
    def __repr__(self):
        return f"<RejectedReservation {self.name}>"
...
def _seed_menu():
    ...
    catalog = [
        ...
        dict(
            ...
            price=500,
            image="images/file_00000000ef40720a9f85f6d7844a9627.png",
        ),
        dict(
            section="iftar",
            category="main-dishes",
            ...
            price=400,
            image="images/file_00000000a1a8720a8c110a51990507f2.png",
        ),
        dict(
            section="iftar",
            category="main-dishes",
            sort_order=3,
            name_en="Meat Taaine with Onions",
            name_ar="تاجن لحم بالبصل",
            ...
            price=450,
            image="images/meatWithOnions.png",
        ),
        dict(
            section="iftar",
            category="main-dishes",
            sort_order=4,
            name_en="Grilled Kofta Plate",
            ...
            price=400,
            image="images/grillesKofta.png",
        ),
        dict(
            section="iftar",
            category="main-dishes",
            sort_order=5,
            name_en="Shish Tawook Plate",
            ...
            price=400,
            image="images/shish.png",
        ),
        dict(
            section="iftar",
            category="main-dishes",
            sort_order=6,
            name_en="Chicken Liver & Hearts",
            name_ar="كبدة ودوانات الدجاج",
            ...
            price=350,
            image="images/ch liver.png",
        ),
        dict(
            section="iftar",
            category="main-dishes",
            sort_order=7,
            name_en="Beef Liver Plate",
            name_ar="طبق كبدة البقر",
        ...
        dict(
            ...
            category="foul",
            sort_order=1,
            name_en="Spicy Foul with Oil",
            name_ar="فول حار بالزيت",
            ...
            price=50,
            image="images/spicyFoul.png",
        ),
        dict(
            section="suhoor",
            category="foul",
            sort_order=2,
            name_en="Foul with Butter",
            name_ar="فول بالزبدة",
            ...
            price=50,
            image="images/butterFoul.png",
        ),
        dict(
            section="suhoor",
            category="foul",
            sort_order=3,
            name_en="Foul with Olive Oil",
            name_ar="فول بزيت الزيتون",
        ...
        dict(
            ...
            category="foul",
            sort_order=4,
            name_en="Alexandrian Foul",
            name_ar="فول إسكندراني",
        ...
        dict(
            ...
            price=80,
            image="images/spanishOmlette.png",
        ),
        dict(
            section="suhoor",
            category="egg",
...
def menu():
    def q(s, c):
        return (
            MenuItem.query.filter_by(section=s, category=c, is_active=True)
...
# ── NEW: Reservation thank-you page ──────────────────────────────────────────
@app.route("/reservation/confirmed")
def thank_you_page():
    name = session.pop("ty_name", "")
    date = session.pop("ty_date", "")
    time = session.pop("ty_time", "")
    return render_template("thankyou.html", name=name, date=date, time=time)
...
def handle_reserve():
    res_data = {
        ...
        for k in ("name", "email", "phone", "date", "time", "message")
    }
    # NOTE: we no longer write to the DB here. The reservation only becomes
    # a real DB row once the Power Automate approval flow calls back to
    # /webhook/approve (see below). Until then it only lives inside the
    # Power Automate flow / approval email.
    try:
        pa_url = (
        ...
        print(f"❌  {e}")
    # ── pass booking details to the thank-you page via session ──
    session["ty_name"] = res_data.get("name", "")
    session["ty_date"] = res_data.get("date", "")
    session["ty_time"] = res_data.get("time", "")
    return redirect(url_for("thank_you_page"))


# ── Power Automate webhooks ───────────────────────────────────────────────────
# Called by the approval flow's HTTP action once the manager clicks
# Approve/Reject on the notification email. No auth on these for now since
# the URL is only shared via the ngrok tunnel + the Power Automate flow itself.

REQUIRED_RES_FIELDS = ("name", "email", "phone", "date", "time")


def _reservation_payload():
    """Pull reservation fields out of an incoming webhook call. Power Automate's
    HTTP action may or may not set Content-Type: application/json, so we try
    JSON first and fall back to form-encoded."""
...
def webhook_approve():
    data = _reservation_payload()
    if not all(data.get(f) for f in REQUIRED_RES_FIELDS):
        return {"ok": False, "error": "missing required field(s)"}, 400
    try:
        db.session.add(Reservation(**data))
        db.session.commit()
    except Exception as e:
        db.session.rollback()
        print(f"❌  webhook_approve: {e}")
        return {"ok": False, "error": str(e)}, 500
    return {"ok": True}, 200


@app.route("/webhook/reject", methods=["POST"])
def webhook_reject():
    data = _reservation_payload()
    if not all(data.get(f) for f in REQUIRED_RES_FIELDS):
        return {"ok": False, "error": "missing required field(s)"}, 400
    try:
        db.session.add(RejectedReservation(**data))
        db.session.commit()
    except Exception as e:
        db.session.rollback()
        print(f"❌  webhook_reject: {e}")
        return {"ok": False, "error": str(e)}, 500
    return {"ok": True}, 200
...
@app.route("/lc-portal-hrg", methods=["GET", "POST"])  # ← CHANGED from /admin/login
def admin_login():
...
def admin_menu_page():
    return render_template(
        ...
        items=MenuItem.query.order_by(
            MenuItem.section, MenuItem.category, MenuItem.sort_order
        ).all(),
        sections=SECTIONS,
...
def admin_menu_edit(item_id):
    ...
    try:
        ...
        item.desc_en = request.form.get("desc_en", "").strip()
        item.desc_ar = request.form.get("desc_ar", "").strip()
